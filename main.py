...
for t in range(start,end):
    ...
    x = np.hstack( ( np.array(windows_hi[t-window-1]), np.array(windows_lo[t-window-1]), np.array(windows_opn[t-window-1]),
                    np.array(windows_close[t-window-1]), exp_lo[t-1],exp_hi[t-1], exp_opn[t-1], exp_close[t-1],
                    np.array(bb_low_h[t-window-1]) , np.array(bb_low_l[t-window-1]),np.array(bb_hi_h[t-window-1]) , np.array(bb_hi_l[t-window-1]),
                    np.array(bb_opn_h[t-window-1]) , np.array(bb_opn_l[t-window-1]),np.array(bb_close_h[t-window-1]) , np.array(bb_close_l[t-window-1]),opn[t] ))
